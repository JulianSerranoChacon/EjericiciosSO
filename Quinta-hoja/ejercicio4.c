...
void* producer(void* arg) {
    ...
    for (int i = 0; i < NUM_ELEMENTS; i++) {
        ...
        // producir elemento
        int item = id * 100 + i;  // ejemplo: id del productor + contador
        ...
        pthread_cond_signal(&can_consume);  // aviso a consumidores
...
void* consumer(void* arg) {
    ...
    while (1) {
        ...
        // esperar hasta que haya elementos para consumir
        ...
        pthread_cond_signal(&can_produce);  // aviso a productores
        pthread_mutex_unlock(&mutex);
    }
...
int main(int argc, char *argv[]) {
    ...
    // los consumidores normalmente correrían infinito
    // opcional: cancelar hilos de consumo después de producción
    for (int i = 0; i < C; i++)
